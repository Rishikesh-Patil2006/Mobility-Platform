...
let mockReviewsStore: ReviewItem[] = [
  {
    id: 'rev-1',
    vendorId: 'vendor-1',
...
export const fetchReviews = async (vendorId = 'vendor-1'): Promise<ReviewItem[]> => {
  return [...mockReviewsStore];
};

export const addOrEditReply = async (reviewId: string, replyText: string): Promise<ReviewItem | null> => {
...
export const postVendorReply = addOrEditReply;

export const deleteReply = async (reviewId: string): Promise<ReviewItem | null> => {
  const idx = mockReviewsStore.findIndex((r) => r.id === reviewId);
  if (idx > -1) {
    mockReviewsStore[idx] = {
      ...mockReviewsStore[idx],
...
export const deleteVendorReply = deleteReply;

export const syncReviewsToBackend = async (): Promise<void> => {
  try {
